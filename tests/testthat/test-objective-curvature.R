test_that("Gaussian nnet objectives are reconstructed exactly", {
  ...
  for (decay in c(0, 0.2)) {
    ...
    model <- suppressWarnings(interpretnn(fit, data = data, response = "continuous"))
...
test_that("binary entropy objectives are reconstructed exactly", {
  ...
  for (decay in c(0, 0.2)) {
    ...
    model <- suppressWarnings(interpretnn(fit, data = data, response = "binary"))
    probability <- pmin(
...
test_that("penalty removal and likelihood scaling follow the documented convention", {
  ...
  binary_model <- suppressWarnings(interpretnn(binary_fit, data = binary_data))
  expect_equal(
