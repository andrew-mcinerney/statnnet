...
plotnn <-
            ...
            ...)
  {
    ...
    p <- net$n[1]
    q <- net$n[2]
    net$weights <- list(list(
      matrix(net$wts[1:((p + 1) * q)],
             nrow = p + 1, ncol = q),
      matrix(net$wts[((p + 1) * q + 1):length(net$wts)],
             nrow = q + 1, ncol = 1)))
    p_values <- net$wald_sp_p
    sig <- p_values < alpha
    ...
    if (is.null(rep)) {
      ...
      if (information)
        grid::grid.text(paste("RSS: ", round(net$value, rounding),
