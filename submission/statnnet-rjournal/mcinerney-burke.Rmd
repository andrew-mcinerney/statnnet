...
abstract: >
  ...
  groups of input weights, fast average observed-row predictive finite-difference
  summaries, partial-dependence covariate-effect plots and contrasts with uncertainty
  estimates, and diagnostic information concerning optimisation and covariance estimation. The
...
bibliography: mcinerney-burke.bib
editor_options: 
  markdown: 
    wrap: sentence
---
...
For this purpose, \pkg{statnnet} provides two related but distinct finite-difference estimands.
The partial-dependence PCE is used for effect curves and can also be averaged to a scalar contrast; the default \code{summary()} instead reports a faster observed-row predictive finite difference.

### Partial-dependence PCE curves
...
### Scalar partial-dependence and observed-row effects

The partial-dependence PCE curve can be reduced to a single point-estimate effect by averaging it over the observed values of the focal covariate: $$
   \hat{\tau}^{\mathrm{PD}}_j
   =
   \frac{1}{n}
   \sum_{i=1}^n
...
For continuous covariates, \code{summary(object)} defaults to \code{effects = "rowwise"} and reports the faster average observed-row predictive finite difference
$$
...
This estimand changes the focal covariate within each observed row, averages the resulting prediction differences, and requires only $O(n)$ prediction evaluations.
...
The interface and terminology follow this distinction: \code{summary(object)} returns $\hat{\tau}^{\mathrm{row}}_j$, \code{summary(object, effects = "partial")} returns $\hat{\tau}^{\mathrm{PD}}_j$, and \code{pce()} and \code{plot()} return the partial-dependence curve $\hat{\beta}(x^{(j)}, d)$.
Neither scalar estimand is labelled as the other in the printed output.
Together, the Wald-type summaries and these effect estimands provide the two pieces of information usually sought from regression-style output: whether a covariate appears to contribute to the fitted model, and how the fitted response changes with that covariate.
...
Model-agnostic interpretability packages provide an important first comparison.
\CRANpkg{pdp} computes partial-dependence and individual-conditional-expectation curves for many fitted model classes [@pdp], while \CRANpkg{iml} [@iml], \CRANpkg{DALEX} [@DALEX], and \CRANpkg{vip} [@vip] provide broader collections of partial dependence, accumulated local effects, variable importance, surrogate models, or Shapley-value explanations.
These tools are valuable because they compare prediction functions across many model classes, but they are not designed around the likelihood, weight ordering, and curvature of a particular \code{"nnet"} fit.

Two packages are closer in their explicit support for neural-network objects.
\CRANpkg{NeuralNetTools} provides methods for \code{"nnet"} and other neural-network classes, including network diagrams, connection-weight variable-importance measures, and Lek profile sensitivity plots [@beck2018neuralnettools].
\CRANpkg{NeuralSens} calculates derivatives of network outputs with respect to inputs and summarises their distribution across the training observations; it supports \code{"nnet"} and several other multilayer-perceptron implementations [@pizarroso2022neuralsens].
These packages provide useful complementary descriptions of network structure, importance, and local sensitivity.
Their primary targets differ from the finite-difference partial-dependence and observed-row estimands in \pkg{statnnet}, and they do not reconstruct the \CRANpkg{nnet} likelihood curvature to obtain the grouped parameter Wald summaries used here.

\CRANpkg{cito} is the closest comparison in terms of regression-like effect output.
It fits separate \pkg{torch}-based neural networks and reports permutation importance, average conditional effects, partial-dependence and accumulated-local-effect plots; bootstrap refits can provide intervals and p-values [@amesoeder2024cito].
Thus, \pkg{cito} has broader architecture and fitting ambitions, whereas \pkg{statnnet} is a post-fit extension for an existing \code{"nnet"} object and uses an analytic covariance approximation without refitting.
The approaches are complementary rather than interchangeable because they operate on different fitted model classes and use different inferential mechanisms.

Table \ref{tab:software-comparison-table} summarises the closest comparisons.
...
The calls below show how the packages target different quantities for the same fitted \code{"nnet"} object; they are deliberately not presented as numerically equivalent estimators.
An evaluated same-fit comparison with \CRANpkg{pdp} is reported in Section \ref{sec:pdp-agreement}.

```{r software-side-by-side, echo = TRUE, eval = FALSE}
...
\CRANpkg{cito} does not consume the fitted \code{"nnet"} object, so an analogous call starts by fitting a separate model with \code{cito::dnn()} and obtains its average conditional effects and importance through \code{summary()}.
Consequently, a numerical comparison with \pkg{cito} would conflate differences in estimand with differences in optimiser, regularisation, architecture, and fitted solution.
...
```{r example-region-factor, echo = FALSE}
...
```{r example-native-summary, echo = TRUE, results = "hide"}
summary(nn)
```
...
The resulting object retains the original \code{"nnet"} fit and the model information required by the statistical methods, including the reconstructed model matrix, response, weight-decay value, covariance estimate, grouped Wald-type summaries, observed-row and partial-dependence effect calculations, and numerical diagnostics.
...
To avoid duplicating the grouped Wald table displayed in the following section, we show its effect component here:
...
The first table reports $\hat{\tau}^{\mathrm{row}}_j$, the average observed-row predictive finite-difference effects defined in Section \ref{sec:fnn-background}, with pointwise standard errors and intervals when covariance estimation is available.
...
The default is therefore explicitly a rowwise estimand, not an average partial-dependence effect.
The more computationally intensive $\hat{\tau}^{\mathrm{PD}}_j$ effects defined in Section \ref{sec:fnn-background} can be requested explicitly:
...
```{r example-pce-bmi, echo = TRUE, fig.width = 6.5, fig.height = 4, out.width = "72%", fig.cap = "Partial covariate-effect plot for body mass index in the insurance data. The plot shows the estimated change in fitted charges associated with a change in standardised body mass index over the observed covariate range.", fig.alt = "A partial covariate-effect plot for body mass index, with body mass index on the horizontal axis and the estimated covariate effect on the vertical axis."}
plot(snn, variable = "bmi", uncertainty = "none")
```

## Numerical agreement with `pdp` {#sec:pdp-agreement}

The comparison below checks the PCE calculation against an independently computed partial-dependence curve for the same fitted \code{"nnet"} object.
We evaluate the partial dependence at two BMI values separated by one sample standard deviation and take their finite difference.
Because both calculations average predictions over the same empirical covariate rows, they target the same quantity and should agree up to numerical precision.

```{r pdp-agreement-table, echo = FALSE, results = "asis"}
comparison_value <- 0
...
```{r example-pce-delta, echo = TRUE, fig.width = 6.5, fig.height = 4, out.width = "72%"}
plot(
  snn,
  variable = "age",
  uncertainty = "delta"
...
```{r example-pce-interaction, echo = TRUE, fig.width = 6.5, fig.height = 4, out.width = "72%"}
plot(
  snn,
  variable = "bmi",
  by = "smokeryes",
...
Both covariate-effect estimands should be interpreted as summaries of the fitted model, not as causal effects.
The partial-dependence PCE averages predictions over the empirical distribution of the remaining covariates at each focal value; the default observed-row effect instead changes the focal value within each observed row before averaging the prediction differences.
They can differ materially under interactions, and neither resolves confounding or a lack of support caused by correlated predictors.
...
The package implements regression-style summaries, Wald-type summaries for individual weights and groups of weights, analysis-of-variance-style summaries for covariates, average observed-row predictive finite differences, partial-dependence PCE plots and contrasts with uncertainty estimates, architecture diagrams annotated with statistical summaries, and prediction methods for new observations.
