...
\abstract{%
The nnet package provides a stable and widely used implementation of single-hidden-layer feedforward neural networks in R, but its output is primarily concerned with fitting and prediction. The statnnet package extends fitted nnet objects with regression-style statistical summaries for small, parsimonious networks. It provides Wald-type summaries for individual weights and groups of input weights, fast average observed-row predictive finite-difference summaries, partial-dependence covariate-effect plots and contrasts with uncertainty estimates, and diagnostic information concerning optimisation and covariance estimation. The scope is deliberately restricted to supported models fitted using nnet; the package is not intended to provide general-purpose inference for deep or otherwise highly parameterised neural networks. This paper describes the statistical quantities computed by statnnet, explains the package design and its limitations, and demonstrates the workflow using an applied regression example.
}
...
\subsection{Covariate-effect estimands}\label{covariate-effect-estimands}
...
\subsubsection{Partial-dependence PCE curves}\label{partial-dependence-pce-curves}
...
\subsubsection{Scalar partial-dependence and observed-row effects}\label{scalar-partial-dependence-and-observed-row-effects}
...
\CRANpkg{pdp} computes partial-dependence and individual-conditional-expectation curves for many fitted model classes \citep{pdp}, while \CRANpkg{iml} \citep{iml}, \CRANpkg{DALEX} \citep{DALEX}, and \CRANpkg{vip} \citep{vip} provide broader collections of partial dependence, accumulated local effects, variable importance, surrogate models, or Shapley-value explanations.
...
\CRANpkg{NeuralNetTools} provides methods for \code{"nnet"} and other neural-network classes, including network diagrams, connection-weight variable-importance measures, and Lek profile sensitivity plots \citep{beck2018neuralnettools}.
\CRANpkg{NeuralSens} calculates derivatives of network outputs with respect to inputs and summarises their distribution across the training observations; it supports \code{"nnet"} and several other multilayer-perceptron implementations \citep{pizarroso2022neuralsens}.
...
It fits separate \pkg{torch}-based neural networks and reports permutation importance, average conditional effects, partial-dependence and accumulated-local-effect plots; bootstrap refits can provide intervals and p-values \citep{amesoeder2024cito}.
...
\caption{\label{tab:software-comparison-table}Closest R packages for neural-network interpretation and effect summaries.}
\centering
\begin{tabular}[t]{l|l|l}
\hline
Package & Fit or interface & Primary output\\
\hline
statnnet & Existing `nnet` fit. & Row/PD effects; analytic grouped Wald.\\
\hline
pdp & Many model classes. & PD and ICE curves.\\
\hline
NeuralNetTools & `nnet` and other NNs. & Plots, weight importance, Lek profiles.\\
\hline
NeuralSens & `nnet` and other MLPs. & Input-derivative sensitivities.\\
\hline
cito & Own `torch` fits. & ACE, PDP/ALE, importance; bootstrap.\\
\hline
\end{tabular}
\end{table}
...
\hline
Covariate-effect description & Not provided. & Rowwise effects and PD plots.\\
\hline
...
#> 1       age average observed-row finite difference -1.724186e-16    1
#> 2   sexmale                                  1 - 0  1.000000e+00    1
#> 3       bmi average observed-row finite difference -2.097737e-16    1
#> 4  children average observed-row finite difference -8.764235e-17    1
#> 5 smokeryes                                  1 - 0  1.000000e+00    1
#> 6    region                  northwest - northeast            NA   NA
#> 7    region                  southeast - northeast            NA   NA
#> 8    region                  southwest - northeast            NA   NA
#>      estimate  std_error    conf_low    conf_high
#> 1  0.31363077 0.01112587  0.29182447  0.335437066
#> 2 -0.04643133 0.02006795 -0.08576379 -0.007098873
#> 3  0.14386456 0.01080637  0.12268446  0.165044654
#> 4  0.05046739 0.00994552  0.03097453  0.069960252
#> 5  1.97032148 0.02579351  1.91976713  2.020875825
#> 6 -0.03099674 0.02894994 -0.08773758  0.025744088
#> 7 -0.07110343 0.02911544 -0.12816865 -0.014038217
#> 8 -0.09547673 0.02876126 -0.15184776 -0.039105687
\end{verbatim}
...
{\centering \includegraphics[width=0.72\linewidth,alt={A neural-network architecture diagram with eight input nodes, two hidden nodes, one output node, and directed connections between layers.}]{mcinerney-burke_files/figure-latex/example-plotnn-1} 
...
{\centering \includegraphics[width=0.72\linewidth,alt={A partial covariate-effect plot for body mass index, with body mass index on the horizontal axis and the estimated covariate effect on the vertical axis.}]{mcinerney-burke_files/figure-latex/example-pce-bmi-1} 
...
\subsection{\texorpdfstring{Numerical agreement with \texttt{pdp}}{Numerical agreement with pdp}}\label{sec:pdp-agreement}
...
\begin{tabular}{l|r|r}
\hline
Method & Estimate & Absolute difference from statnnet\\
\hline
statnnet PCE & 0.1720832 & 0\\
\hline
finite difference of pdp PD & 0.1720832 & 0\\
\hline
\end{tabular}
...
\begin{center}\includegraphics[width=0.72\linewidth]{mcinerney-burke_files/figure-latex/example-pce-delta-1} \end{center}
...
\begin{center}\includegraphics[width=0.72\linewidth]{mcinerney-burke_files/figure-latex/example-pce-interaction-1} \end{center}
